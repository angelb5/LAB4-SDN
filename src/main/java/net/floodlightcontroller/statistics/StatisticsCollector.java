...
public class StatisticsCollector implements IFloodlightModule, IStatisticsService {
					...
					for (OFPortStatsEntry pse : psr.getEntries()) {
						...
						if (portStats.containsKey(npt) || tentativePortStats.containsKey(npt)) {
							...
							if(spb.getBitsPerSecondTx().getValue()>portTxThreshold){
								log.info("Se ha superado el threshold Tx: Threshold: "+portTxThreshold+", Bandwidth Tx: "+spb.getBitsPerSecondTx().getValue());
							}

							if(spb.getBitsPerSecondRx().getValue()>portRxThreshold){
								log.info("Se ha superado el threshold Rx: Threshold: "+portRxThreshold+", Bandwidth Rx: "+spb.getBitsPerSecondRx().getValue());
							}
	...
	public void init(FloodlightModuleContext context)
		...
		portRxThreshold = Integer.parseInt(config.get("portRxThreshold"));
